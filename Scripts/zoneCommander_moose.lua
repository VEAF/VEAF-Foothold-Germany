...
do
	...
	if zoneName then
		...
		if not grname:find("Red SAM") and not grname:find("bluePD") and not grname:find("blueHAWK") and not grname:find("bluePATRIOT") then
...
function upgradeRedZones()
    ...
    local function process(idx)
        ...
        if zname then
            ...
            if z then z:MakeZoneSideAndUpgraded() end
            timer.scheduleFunction(function()process(idx+1)end,{},timer.getTime()+15)
        end
    end
    process(1)
end
					...
					if staticObj then
						...
						if not tostring(upg):find("dismounted") then
							...
							if gr then
								...
								trigger.action.outTextForCoalition(2,'Upgrade group '..tostring(upg)..' missing, skipped.',10)
  ...
  function RunwayHandler:OnEventShot(EventData)
	...
	self:StopTrack() return	end
      for i, z in ipairs(hitZones) do
        if z:IsVec3InZone(p) and not done[z] then
...
function buildCapControlMenu()
	...
	if not (capActive or casActive or seadActive or decoyActive or bomberActive or StructureActive) then
		if capControlMenu then
			missionCommands.removeItemForCoalition(2, capControlMenu)
			capControlMenu = nil
		end
