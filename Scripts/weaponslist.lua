...
"FA-18FT","EA-18G","F-22A","FA-18E","B-52H","FA-18F","FA-18ET","F15EX"}
    ...
    if Era == 'Coldwar' then
        ...
        if airbase then
            ...
            if storage then
                ...
                for _,plane in ipairs(allowedPlanes) do storage:SetItem(plane,1000) end
        ...
        for _, airbaseName in ipairs(airbaseList) do
            ...
            if storage then
                for _, wp in ipairs(restockWeapons) do
                    ...
                    if (not okAmt) or (amt == nil) or (amt >= 0 and amt < 10000) then
                        local ok = pcall(function() storage:SetItem(wp, 10000) end)
                        if ok then
                            env.info('ADDED '..airbaseName..' '..(type(wp) == 'table' and '{'..table.concat(wp, ',')..'}' or tostring(wp))..' = 10000')
                        else
                ...
                for _, ac in ipairs(restockAircraft) do
                    ...
                    if (not okAmt) or (amt == nil) or (amt >= 0 and amt < 10000) then
                        local ok = pcall(function() storage:SetItem(ac, 10000) end)
                        if ok then
                            env.info('ADDED '..airbaseName..' '..(type(ac) == 'table' and '{'..table.concat(ac, ',')..'}' or tostring(ac))..' = 10000')
                        else
...
--[[ SCHEDULER:New(nil,function()
  local st = STORAGE:FindByName("Ramon Airbase") if not st then return end
  local ac,_,wp = st:GetInventory()
  UTILS.PrintTableToLog(ac)
end,{},5,0) ]]
...
--SCHEDULER:New(nil, DumpWeapons, {}, 5, 0)     -- all weapons (ws[1]==4)
